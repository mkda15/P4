...
Fourier series are only one of a large class of interesting and useful infinite series expansions that are based on so-called \textit{orthogonal systems} or \textit{orthogonal sets} of functions \cite{page 62, FAA} \martin{Dette er faktisk kopieret direkte fra indledningen til kapitel 3 i Folland. Er det et problem? \textregistered}. This section studies the orthogonality of the Fourier series and is inspired by \cite{pages 62-77, FAA} and the lecture notes from the course \textit{Anvendt Harmonisk Analyse}.
...
The final question in this section is when an ortonormal family in $\mathcal{L}^2(a,b)$ actually is an ortonormal basis for $\mathcal{L}^2(a,b)$. An ortonormal family $\{\phi_n\}_{n=1}^\infty$ satisfies $f = \sum_{n=-\infty}^\infty \langle f, \phi_n \rangle \phi_n \ \forall f \in \mathcal{L}^2(a,b)$ as in \eqref{eq:ort_fam}. The following theorem is inspired by \cite{page 77, FAA}.
