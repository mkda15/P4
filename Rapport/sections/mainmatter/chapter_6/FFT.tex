...
\trine{man kunne evt. indledningsvist gøre opmærksom på hvordan der refereres til de figurere}The FFT algorithm in this project is based on the decimation-in-time (DIT) FFT algorithms as they are the most efficient FFT algorithms. The idea is to decompose the computation into successively smaller DFT computations while exploiting both the symmetry and the periodicity of the complex exponential $W_N^{kn}$ \cite{page 755, DTSP}. It is assumed that $N$ is an integer power of 2, i.e. $N = 2^\nu$ for $\nu \in \mathbb{N}$.
\\ \\
...
\end{tikzpicture}
\caption{Flow graph of an $8$-point DFT decomposed into two $4$-point DFTs. Circles symbolize a summation of the lines meeting.}
\label{fig:4}
\end{figure}
...
\\
In general, there are $\log_2(N)$ stages of computation (which is 3 in figure \ref{fig:fft_flow}), and each of these involves a set of $N/2$ 2-point DFTs (the so-called butterflies). Notice that in figure~\ref{fig:fft_flow} these butterflies does not intersect at the first stage, at the second stage the butterflies intersect pairwise and at the last stage all of the butterflies intersect \martin{Har erstattet $N$ med 8 i denne figur, fordi det er et specifikt eksempel på FFT'en, hvorimod den anden figur er mere generel. \textregistered}.
\begin{figure}[H]
\centering
\begin{tikzpicture}
...
\end{tikzpicture}
\caption{Flow diagram of an 8-point DFT decomposed into 4 2-point DFTs with the FFT. Circles symbolize summation of the meeting lines.}
\label{fig:fft_flow}
\end{figure}
