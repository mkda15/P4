...
Figure \ref{fig:discreteSTFT} shows the discrete STFT illustrated by windowing of a signal by adjacent windows with different overlaps. Clearly, no overlap results in loss of data. On the other hand, a rather large overlap results in more computations \martin{Nyt. \textregistered}.
...
\end{subfigure}
\caption{Illustrations of \textbf{(a)} two adjacent windows in the discrete STFT with no overlap windowing the signal and \textbf{(b)} two adjacent windows in the discrete STFT with 50\% overlap windowing the signal.}
