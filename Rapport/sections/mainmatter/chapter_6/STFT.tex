...
Let $f,g \in \mathcal{L}^2(\mathbb{R}^d)$. For a fixed window function $w \neq 0$ the short-time Fourier transform of a function $f$ with respect to $g$ is defined as:
\begin{align}
X(\tau,\omega) = \int_{\mathbb{R}^d} f(t) w(t - \tau) \text{e}^{-j \omega \cdot t} dt \quad \textnormal{for} \quad \tau,\omega \in \mathbb{R}^d.
\end{align}
\end{definition}
...
Let $f,g\in\ell^2$. For a fixed window sequence $w \neq 0$ the discrete short-time Fourier transform of a sequence $f$ with respect to $w$ is defined as
\begin{align}
X[n, \omega] = \sum_{m=-\infty}^{\infty} f[n+m]w[m]\text{e}^{-j\omega m}.
\end{align}
\end{definition}
...
\begin{align*}
|X[n,\omega]|^2 = \left| \sum_{m=-\infty}^{\infty} f[n+m]w[m]\text{e}^{-j\omega m} \right|^2
\end{align*}
