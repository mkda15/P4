...
\martin{Har slettet det oprindelige og skrevet et alternativ. Det oprindelige kan gemmes som en anden fil. \textregistered}
The discrete Fourier transform (DFT) is the discrete time variant of the Fourier transform and can be used to transform samples from the time domain into the frequency domain. Multiple parts of the DFT are analog to the continuous version \cite{DFT_OX}. This section is inspired by the lecture notes from the course \textit{Anvendt Harmonisk Analyse} and \cite{page 252-264, FSP}.
\\ \\
...
\\ \\
A fundamental property of linear shift invariant (LSI) systems is that they all have unit-modulus complex exponential sequences as eigensequence \cite{FSP} \martin{Det her med LSI-systemer skal som minimum omformuleres. Har vi et sidetal på det? \textregistered}.
\\ \\
...
The DFT is closely connected to circular convolution, which uses the modulo function $\langle k\rangle_N$. It is possible to write $k = LN+r$ for $0 \leq r < N$ and $L \in \mathbb{Z}$ in a unique way, and the modulo function finds the remainder $r$ from division of $N$ by $k$ \martin{Har vi en kilde på ``division of $N$ by $k$''? Synes ikke helt, at det passer ind. \textregistered}:
\begin{align*}
\langle k \rangle_N = r.
\end{align*}
...
This shows that a circular convolution operator $H$ has an eigensequence that is the twiddle factor. \martin{Mangler for dette afsnit: vi skal være enige om notationen for indekser ved cirkulær foldning i for eksempel definition 6.2.5. Derudover skal delene om LSI-systemer og sidste del af afsnittet om egenfunktioner omformuleres og kobles bedre sammen med resten af afsnittet. Endelig bør der angives sider ved kildehenvisningerne, hvor det er muligt. \textregistered}.