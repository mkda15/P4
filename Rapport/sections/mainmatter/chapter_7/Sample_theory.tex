...
\end{definition}
To calculated the variances of the signal and noise the standard deviation $\sigma$ can be calculated and if the input signal has mean = 0 the standard deviation of the signal can be calculated by the root mean square (RMS) of the signal [\textbf{page 228}\cite{DTSP}] which is defined as\trine{det ser tosset ud med den kilde der, rykkes op til første linje ik?} \martin{Ydermere skal kilderne citeres ligesom i resten af rapporten, altså blot med sidetallet inde i cite-funktionen (og ikke noget med fed skrift). \textregistered}
