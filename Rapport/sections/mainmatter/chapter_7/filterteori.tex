...
A digital filter is characterised as a LTI system, previous defined by equation \eqref{eq:LTI_diff_equation_finite}. As described the system is completely characterized by its corresponding impulse response $h[n]$. In terms of the LTI system as a filter the impulse response determine the output $y[n]$ that is how much of the input sequence that passes the filter. \\
...
As described casual systems with generalized linear phase is wanted. This is possible to guaranty by using specific types of FIR filter.\\
...
A type 1 FIR filter is characterised by having a symmetric impulse response and $M$ being an even integer. By applying the the symmetry condition to the definition of the Fourier transformation the frequency response is defined by   
