...
In this section the process of designing digital filter are described. There will be a distinguished between techniques for IIR and FIR filters.\\ The section is concluded by a comparison of FIR and IIR filters.\\  
As described ideal filters are not computable. Hence the process of designing a filter is based on approximation of the frequency response - made by computable polynomials - on behave off a set of specification for the filter. Different design methods exist to archive this.     
...
\item[2.] Hereby compute a "prototype" by a continuous time system $H_c(j\Omega)$ that approximates the given properties. Done as a \textit{Butterworth filter}  
\item[3.] Transform the "prototype" into the discrete time filter $H(e^{j\omega})$. Done with the \textit{Bilinear trasformation}. 
\end{itemize}
...
Methods for defining a continuous time system that fits the specification are establish as known continuous time filter designs as the Butterworth filter which is a lowpass filter approximation. For the Butterworth filter the squared magnitude is defined as follows.
...
Design of FIR filters are in contrast to IIR filters based on approximating the desired discrete-time frequency response directly. By this method it is possible to guarantee a general linear phase of the system. The window method will be described in this section.   
...
\end{matrix}\right.
\end{align}        
where $\alpha=\frac{M}{2}$ and $I_0(\cdot)$ the zero'th order modified Bessel function of the first kind.\cite{DTSP, p.474-476} Increasing $\beta$ tapers the window in the time domain, which results in smaller side lopes of the Fourier transform. Increasing $M$ causes the width of the main lope to increase though without changing the peak amplitude of the side lopes. This i illustrated on figure \ref{?} and \ref{?} respectively\trine{2 figure}.\\ 
...
To conclude this section about design of digital filters a comparison of IIR and FIR filter will be stated considering the advantages and disadvantages of the different systems. \\ 
\\
The process of designing a IIR filter - presented in this section - is rather simple when the desired properties fits an establish design method such as a Butterworth filter. The required order of the filter is computable and the transformation to the discrete-time domain is to be done straight forward. Further, considering the implementation, this leads to non iterative algorithms. By this method the magnitude response is the only parameter to be specified, hence specific requirements for e.g. phase response or group delay are not guaranteed to be fulfilled by this kind of IIR filter.\\ \\
Considering a FIR filter the main advantages - compared to FIR filters - is the possibility of getting a precise generalized linear phase. The generalised linear phase is important in order to preserve the wave form of the original signal.  
The window method also straight forward to apply, though it is not pre-defined design equation, and so more or less iterations are often necessary before desired specifications is achieved. \\
...
\\
By this it is clear that the choice of filter depends on the specific case, and how different aspects are weighted. IIR filter have many advantages in implementation and efficiency which makes it preferable in cases where a need of generalized linear phases is less significant, though in other cases linear phase by a FIR filter can be worth the extra computations.       
