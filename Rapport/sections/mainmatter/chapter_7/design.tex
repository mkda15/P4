...
Methods for defining a continuous-time system that fits the specification are establish as known continuous-time filter designs. The \textit{Butterworth filter} which is a lowpass filter approximation will now be described along with the ?? \trine{udfyld} transformation.\\
...
\end{align}
As the order $N$ of the filter increases the characteristics become sharper and the filter approximates the ideal lowpass filter. This is illustrated on figure \ref{fig:butter}.       
\begin{figure}[H]
    \centering
...
\end{align} 
\trine{er det nødvendigt at medtage s og poler eller skal vi bare opskrive det endelige H(s) tidligere}
... \\
\\
When the continuous-time filter $H_c(s)$ is defined the next step is the transformation from continuous-time to discrete-time. This can be done i several way.  \\
\\
... 
\\
