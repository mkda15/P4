...
To study the behavior of $x_s(t)$ in the frequency domain the Fourier transform is used. The Fourier transform of $s(t)$ is as follows:
\begin{equation}\label{eq:fourier_impulse}
\hat{s}(j\omega_s)=\frac{2\pi}{T}\sum_{k=-\infty}^{\infty}(j\omega-kj\omega_s)
\end{equation}
where $j\omega$ is the complex frequency (radians pr. second).\\\\
...
\end{equation}
\eqref{eq:samp_freq} mathematically samples $x_c(t)$ in the frequency domain. It is furthermore useful in the below analysis.
\subsection{Aliasing}\label{sec:aliasing}
From \eqref{eq:samp_freq} it is seen that $\hat{x}_s(j\omega)$ consists of copies of $\hat{x}_c$ repeated at whole multiples of $j\omega_s$ on both sides of the origin - this is seen in figure \ref{fig:}.
\begin{figure}
\centering
\begin{tikzpicture}
\begin{axis}[
