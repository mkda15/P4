This chapter describes the mathematical tools necessary to describe and understand the theory described in the following chapters. This project deals with functions and sequences in the so-called $\mathcal{L}^p$ and $\ell^p$ spaces, respectively. The following definitions are inspired by \cite{chapter 5-6, FSE2010} and \cite{chapter 2, FSP} and only considers a subset $D \subset \mathbb{R}$ due to the focus of this project. However, similar definitions also hold for all of $\mathbb{R}$ \martin{Overvejelser til dette kapitel: denne fil indeholder et alternativ til den anden fil i mappen. Overskriften kan måske ændres til noget i retning af ``Mathematical tools'', så det er lidt mere generelt og beskrivende. Man kan eventuelt også inkludere definitioner af Banach- og Hilbert-rum ud fra Cauchy-følger, norm og så videre (side 48 + 65, FSE2010). Derudover er definition 5.0.5 tilføjet, da vi egentlig bruger begreberne heri i løbet af rapporten, men den er tilpasset rapporten i forhold til kilden, og vi skal spørge Henrik, om det er i orden. Endelig er det lidt usikkert, hvor relevante alle begreberne i definition 5.0.6 er. \textregistered}.
...
The following definition defines linear operators on $\mathcal{L}^2(\mathbb{R})$ \cite{page 120, FSE2010}\trine{gentagelser fra kap 4.}.
