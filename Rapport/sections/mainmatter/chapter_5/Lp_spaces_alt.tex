...
The following definition defines linear operators on $\mathcal{L}^2(\mathbb{R})$ \cite{page 120, FSE2010} \martin{Vi bruger egentlig begreberne i denne definition igennem rapporten, og henvisning til denne definition i løbet af rapporten vil derfor binde rapporten bedre sammen. Bemærk: punkt nummer 2 og henvisningerne til denne definition rundt omkring i rapporten er tilpasset rapporten, og vi skal spørge Henrik, om det er i orden. \textregistered}.
...
A definition similar to definition \label{def:Lpspace} is given for $\ell^p$ which deals with sequences:
...
Eventuelt: definition af Banach- og Hilbert-rum ud fra Cauchy-følger, norm og så videre (side 48 + 65, FSE2010).