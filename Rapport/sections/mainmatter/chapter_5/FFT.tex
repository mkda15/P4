...
\\ \\
The Fast Fourier Transform is a special algorithm for fast computation of the DFT. In the following, the $N \times N$ DFT matrix is denoted by $F_N$. In section \ref{sec:DFT} the DFT of length $N$ was defined as:
\begin{align*}
...
The $N$-point DFT $X[k]$ defined in section \ref{sec:DFT} is thus $X[k] = E[k] + O[k]$ for $k = 0, 1, \dots, N/2-1$ on the right-hand side and for $k = 0, 1, \dots N-1$ on the left-hand side. $E[k]$ and $O[k]$ are $N/2$-point transforms, which are implicitly periodic with period $N/2$, and they can thus be 	evaluated over $k = 0, 1, \dots, N-1$ instead of $k = 0, 1, \dots, N/2-1$. Therefore, the $X[k]$ may be written as:
