...
This section describes \textit{The Fast Fourier Transform}, also known as the FFT. The section is inspired by \cite{page 750, DTSP} and \cite{page 303, FSP}.
\\ \\
The Fast Fourier Transform is a special algorithm for fast computation of the DFT. In the following, the $N \times N$ DFT matrix is denoted by $F_N$. In section (insert reference) the DFT of length $N$ was defined as:
\begin{align*}
...
\begin{align*}
X[k] = \sum_{n=0}^{N-1} \left[ (\Re\{x[n]\} \Re\{W_N^{kn}\} - \Im\{x[n]\}\Im \{W_N^{kn}\}) j(\Re{x[n]}\Im \{W_N^{kn}\} + \Im\{x[n]\} \Re\{W_N^{kn}\}) \right]
\end{align*}
...
\\ \\
The FFT algorithm in this project is based on the decimation-in-time (DIT) FFT algorithms as they are the most efficient FFT algorithms. The idea is to decompose the computation into succesively smaller DFT computations while exploiting both the symmetry and the periodicity of the complex exponential $W_N^{kn}$ \cite{page 755, DTSP}. It is assumed that $N$ is an integer power of 2, i.e. $N = 2^\nu$.
\\ \\
In order to compute the Fourier transform $X[k]$, the input sequence $x[n]$ is divided into sequences consisting of the even-numbered points $e[n] = x[2n]$ and the odd-numbered points $o[n] = x[2n+1]$. \martin{Her kan man vise, at $X[k] = E[k] + O[k]$, men i bogen gør de dette ved hjælp af DTFT'en (side 756-757). \textregistered} Therefore, the DFT of $e[n]$ and $o[n]$ is
\begin{align*}
E[k] &= \sum_{n=0}^{N/2-1} x[2n] W_{N/2}^kn, \quad k = 0, 1, \dots, N/2-1 \\
O[k] &= \sum_{n=0}^{N/2-1} x[2n+1] W_{N/2}^kn, \quad k = 0, 1, \dots, N/2-1
\end{align*}

The $N$-point DFT $X[k]$ defined in section (insert reference) is thus $X[k] = E[k] + O[k]$ for $k = 0, 1, \dots, N/2-1$ on the right-hand side and for $k = 0, 1, \dots N-1$ on the left-hand side. $E[k]$ and $O[k]$ are $N/2$-point transforms, which are implicitly periodic with period $N/2$, and they can thus be evaluated over $k = 0, 1, \dots, N-1$ instead of $k = 0, 1, \dots, N/2-1$. Therefore, the $X[k]$ may be written as:
\begin{align} \label{FFT_modulo}
X[k] = E[((k))_{N/2}] + W_N^k O[((k))_{N/2}], \quad k = 0, 1, \dots, N-1
\end{align}

Even though $E[k]$ and $O[k]$ are only computed for $k = 0, 1, \dots, N/2-1$, they are extended periodically. The notation $((k))_{N/2}$ signalizes $k$ modulo $N/2$. In this case, the modulo operation finds the remainder after division of $k$ by $N/2$. Thus, $E[((4))_4]$ equals $E[0]$. Hence, this periodic extension does not require any additional computation. Figure (insert reference) is inspired by \cite{figure 4, DTSP} and shows a flow graph of the decimation-in-time decomposition of an $N$-point DFT into 2 $N/2$-point DFTs (for $N = 8$). Branches entering a node is summed to produce the node variable, and if the transmittance of a branch is different from unity it is an integer power of $W_N$ as indicated next to the arrows. \martin{Indsæt figur nogenlunde magen til figur 4, side 758, her. Tikz \textregistered}
\\ \\
According to \eqref{FFT_modulo} and figure (insert reference), $X[0]$ is obtained by multiplying $O[0]$ with $W_N^0$ and adding this product $E[0]$. Likewise, $X[4]$ is obtained by multiplying $O[((4))_4] = O[0]$ with $W_N^4$ and adding this result to $E[((4))_4] = E[0]$.
\\ \\
