...
Analogues of an ideal highpass or bandpass filter can be defined as illustrated on figure \ref{fig:ideal}.
...
Thus an approximation of an ideal filter needs to be computed by a causal system with generalized linear phase.

\section{FIR and IIR filters} \label{sec:FIR_IIR}
Two classes of filters are essential to identify. For all ideal filters discussed in the previous section the impulse response is defined for $-\infty < n < \infty$. Such a filter is specified as an \textit{infinite impulse response (IIR)} filter. In the case of the impulse response being zero outside a finite interval the filter is referred to as a \textit{finite impulse response (FIR)} filter.
