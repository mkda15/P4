...
\end{matrix}\right.$  \\ \\ \hline
\end{tabular}
\end{table}   \chr{Christian vil gerne have den øverste og nederste linje slettet. Tabellen fylder en hel side nu. Appendix?}
...
By this establishment of the Kaiser window method it is possible to design a FIR filter within a desired specification of $\delta$ with almost no iterations or trial and error. By adjusting the shape parameter $\beta$ it is possible to obtain a window equivalent to each of the windows defined in table \ref{tab:window} with a more narrow transition width, which could make the Kaiser window preferable \martin{Henrik vil gerne have, at vi eventuelt uddyber dette afsnit. Vi er ikke helt sikre på, hvad han mener, men det handler måske om, at der er nogle begreber, som han ikke kender, og det kan afhjælpes ved hjælp af figuren nævnt ovenfor. Skal vi have et eksempel på amplituderesponsen for lavpasfilteret med Kaiser-vinduet ligesom vi har med det rektangulære vindue længere oppe? \textregistered}.       
