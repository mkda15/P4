...
\begin{figure}[H]
    ...
    \caption{Amplitude response of Butterworth filter depending on order $N$ }
    \label{fig:butter}
\end{figure} 
...
Figure \ref{?} illustrates the main and side lopes by a plot of the amplitude response corresponding to the rectangular window \trine{indsæt figur}.
...
In table \ref{tab:window} some commonly used windows are defined. Furthermore, the windows and the corresponding amplitude response in dB are illustrated on figure \ref{fig:window_types} and \ref{?}\trine{figur} .
