...
\end{axis}
\end{tikzpicture}
\caption{Specifications of magnitude response}
\label{fig:scheme}
\end{figure}

The predefined Butterworth filter is a lowpass filter approximation used to fit the filter to the requirements . For the Butterworth filter the squared magnitude response is defined as follows.
...
In table \ref{tab:window} some commonly used windows are defined. Furthermore, the windows and the corresponding magnitude response are illustrated on figure \ref{fig:window_types} and \ref{?}\trine{figur} .
...
As mentioned in section \ref{ADC} an anti-aliasing filter is required in the sampling process. The purpose of this filter is to avoid the aliasing phenomenon which occurs when the sampling theorem \ref{sampling_theorem} is not fulfilled. As stated a signal has to be band limited by the Nyquist frequency $\omega_N$ and since a continuous-time signal such as a sound is not guaranteed to have a bandlimited frequency the anti-aliasing filter is designed to remove the frequencies that are above the Nyquist frequency. \\
...
\trine{flyt denne section til section 7.2}This section about the design of digital filters concludes with a comparison of IIR and FIR filters, which considers the advantages and disadvantages of the different systems. \\ 
\\
