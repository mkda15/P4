...
Writing $x[n]$ in \eqref{eq:DTS} as a sum using the definition of the impulse response and by linearity, it follows that \chr{Har lavet en mindre omskrivning da vi brugte "vi" tidligere.}:
\begin{align}
...
The following definition defines linear operators on the concrete Hilbert space $\mathcal{L}^2(\mathbb{R})$ as defined in definition \ref{def:L2space} \cite{page 120, FSE2010} \martin{Vi bruger egentlig begreberne i denne definition igennem rapporten, og henvisning til denne definition i løbet af rapporten vil derfor binde rapporten bedre sammen. Det bør dog overvejes, om definitionen skal være et andet sted (så der ikke henvises fremad til kapitel 5). Bemærk: punkt nummer 2 og henvisningerne til denne definition rundt omkring i rapporten er tilpasset rapporten, og vi skal spørge Henrik, om det er i orden. \textregistered}.
