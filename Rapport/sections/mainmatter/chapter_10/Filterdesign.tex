...
By the frequency analysis of the specific noise showed in figure \ref{fig:clapping} it is seen that the frequencies of the noise are completely adjacent to the desired passband, which verifies the need of a narrow transitionband. However, the narrower transitionband means the higher order of filter is needed, which gives more computations, and hence it is not realistic to let the width of the transitionband go toward zero.  \\
...
\State $M = (A-8)/(2.285 \cdot f_{tw} \cdot  2\pi)$ \Comment {Filter order, round to upper even int.}
...
\Procedure{Compute ideal impulse response $h_d$}{}
    \For {each integer $i$ in $h_d$}
        \If {$i == \frac{M}{2}$}
        	...
        	\Else 
        		\State  $h_d[i] = \frac{1}{ (\pi (i - \frac{M}{2}))}(\sin(f_{t2} \cdot 2 \pi (i - \frac{M}{2})) - (\sin(f_{t1} \cdot 2 \pi (i - \frac{M}{2}))))$ 
        	\EndIf
	\EndFor
	\State Return $h_d$
\EndProcedure
