...
Due to the method of the Kaiser window described in section \ref{subsec:FIR} the shape parameter $\beta$ and the order of the filter $M$ is determined on behalf of the specified $\delta_1$ end $\delta_2$, as  
\begin{align}
A=&-20\log_{10}(\delta_1) = 40 \\ 
\beta =& \ 0.5824(A-21)^{0.4} + 0.07886(A-21)  \approx 3.4 \\
M =& \ \frac{A-8}{2.285 \cdot \Delta f_t  2\pi}\approx 98294
\end{align}
...
\State $M = 98294$ \Comment {Order of filter} 
\State $\beta = 3.4$ \Comment {Shape parameter}
