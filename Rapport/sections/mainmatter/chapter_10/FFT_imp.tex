...
For $N=2^9$ this gives an approximate relation at $56/1$ between the two, while the actual relation is $103/1$ from table \ref{tab:FTcompare}. From this the computation of the implementet FFT is approximately twice as fast as theoretically assumed compared to the implemented DFT than\trine{NYT, kan vi kommentere på hvorfor det er sådan? Sætning ikke afslutet af den grund??}. \\
\\
