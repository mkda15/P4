...
\label{SPECTROalg}
\begin{algorithmic}[1]
\State $freq= f_s$ \Comment{Set sampling frequency}
\State $stft=STFT(data)$ \Comment{Perform STFT on input $data$}
...
By figure \ref{fig:test_stft} it is concluded that the implementation of the STFT and spectrogram work as intended. Further optimization is possible through of window used in the STFT. 
...
In figure \ref{fig:STFT_test_signal} is seen the resulting STFT of an octatonic scale played on a guitar one string at a time - with use of a Kaiser window of order $2*{11}$ and $\beta = 4$  
\begin{figure}[H]
\centering
