...
\begin{definition}[Dirac delta function]\footnote{The Dirac delta function is defined as a distribution as no function satisfies the characteristics of it. This definition is therefore non-rigorous but will make do for the purposes of this section.}\label{def:dirac}
...
\end{equation}
\end{definition}
The Dirac delta function is sometimes interpreted as a constant 0-function with an infinitely high and infinitely narrow spike at the origin.
\\\\
...
\label{eq:sampling}
\end{align}
\frede{Tjek om det faktisk er Dirac delta. Så skal der måske være et integral.}\eqref{eq:sampling} is a mathematical representation of sampling of the continuous function $x_c(t)$ into the function $x_s(t)$ \cite{DTSP}. Although \eqref{eq:sampling} matehmatically describes sampling, we have $x_s(t)\neq x[n]$ as $x_s(t)$ is defined on the interval between two samples whereas $x[n]$ only takes values at integers $n$. Although $x_s(t)\neq x[n]$ \eqref{eq:sampling} will be useful in the upcoming appliance of the Fourier transform.
...
\end{align}
\frede{Skriv præcis hvilken transformation der bruges. Hvad foreår der ved citationen?}\eqref{eq:samp_freq} mathematically samples $x_c(t)$ in the frequency domain at a rate of $\omega_s$ radians pr. second. 
...
\\\\
From \eqref{eq:samp_freq} it is seen that $X_s(j\omega)$ consists of copies of $X_c$ repeated at whole multiples of $j\omega_s$ on both sides of the origin - seen in figure \ref{fig:copies}.
\begin{figure}[H]
\centering
\begin{tikzpicture}
\begin{axis}[
...
% Drawing the blocks of first filter :
	...
	node [block, right of=input1] (sh) {$Sample$-$and$-$hold$}
