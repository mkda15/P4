...
%
%kvantisering - test? 
\trine{jeg syntes måske en hovedoverskrift med V-model er lige i overkanten}
...
The V-model will be tested from the bottom up\trine{lille omformulering af 1. sætning, det er vel systemet der testes ud fra V modellen}, starting with the smallest components (a unit). This validation phase is called the Unit testing. The test should checks each of the "units" individually and verify, that they are working according to expectations. \\\\
Next up the coherence of the system is tested, and finally the system it self as a single unit is tested. 
\\\\

\textbf{The  units in the model are as follows }\trine{punkt 2 og 3 kunne være ADC, og fjerne den sidste }
\begin{itemize}
	\item Microphone
