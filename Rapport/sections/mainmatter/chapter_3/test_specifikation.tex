...
The algorithm will then be tested on a fixed data set with a known output which illustrates the usability of the spectrogram. The signal to be used is
...
\end{cases}.
\end{align}
The sine waves are then with frequencies of 1, 3 and 3.25 Hz, respectively, and the signal is sampled for $t=75$ seconds at $f_s=4000$ Hz.\\
...
\frede{Alle de ovenstående afsnit skal have testspecifikationerne så de ikke står i kapitel 10.}
