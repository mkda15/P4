...
\\ \\
This project is delimited to deal with a guitar as the musical instrument. A guitar is a string instrument with usually 6 strings from the so-called sadle over the sound hole and fretboard to the machine heads which may be turned to tune the guitar. The fretboard of the guitar consists of frets which are approximately one semitone apart, and they are thus pressed down while bringing the strings in motion with the other hand to create different tones. Eventually, the sound is amplified by the soundboard and the body, and the sound comes out of the sound hole.
\\ \\
The pitch of the tone being played is determined by the length, mass and tension of the strings. The length of the strings is the only variable while playing on a guitar, and in general, cutting the lenght of the string in half doubles the frequency. \cite{WikiGuitar} \cite{AcousticGuitar} 
\\ \\
Graphically, a pure tone would look like a normal sine wave with a certain frequency and amplitude. Such pure tones can easily be found and heard on the internet and can easily drive the listener crazy. A sine wave in its general form is $y(t) = A\sin(2\pi f t + \phi) = A\sin(\omega t + \phi)$ where $A$ is the amplitude, $f$ is the ordinary frequency as described above, $\omega = 2 \pi f$ is the angular frequency (units of radians per second). The concert pitch $A_4$ with frequency $440$ Hz are thus described by the sine wave $y_f(t) = \sin(2 \pi 440 t) = \sin(880 \pi t)$. \\
However, a tone from a guitar is not just a sine wave but has the harmonics added to it. The closest harmonic for the tone $A_4$ is $A_5$ with frequency $880$ Hz and with sine wave $y_h(t) = \sin(2 \pi 880 t) = \sin(1760 \pi t)$. What the guitar actually plays is a blend of the original tone, all the harmonics (which there may be up to five of) and vibrations from the other strings as well. However, for simplicity let's just consider $y(t) = y_f(t) + y_h(t)$ as the output from the guitar playing the tone $A_4$, that is $A_4$ with a single harmonic. \\
Since a period is described by $T = \frac{1}{f}$, the duration of a single period for a sine wave with frequency $440$ Hz is $\frac{1}{440} \approx 0.0023$ seconds. Figure \ref{fig:sine_single} shows 4 periods for the sine wave $y_f(t)$ and 8 periods for the sine wave $y_h(t)$. The output $y(t)$ is shown in figure \ref{fig:sine_sum}.
