...
\end{cases}
\end{align*}
Dette er implementeret i koden således:\todo{f1 eller o1 - rettes}
\begin{lstlisting}
def hd(n,M,f1,f2): 
    hd = np.zeros(len(n))
    ...
    return hd
\end{lstlisting}
Fordi impulsresponsen er endelig skal den ideelle impulsrespons ganges med en vinduesfunktion, og dette Fourier-transformeres herefter for at opnå en realistisk frekvens- og amplituderespons. Der er forskellige muligheder for denne vinduesfunktion såsom det rektangulære vindue, Hamming-vinduet, Hann-vinduet og Blackman-vinduet.$^[$\footnote{Disse vinduer er beskrevet på side 559-560 i \textit{Discrete-Time Signal Processing}, Oppenheim \& Schafer (2014).}$^]$ Implementeringen af vinduerne i koden er ganske simpel, og som eksempel ses implementeringen af Hann- og Hamming-vinduet herunder:
\begin{lstlisting}
...
Grafen for Hamming-vinduet er vist på figur \ref{fig:Hamming} \todo{Overvej dette. \textregistered}.
\begin{figure}[H]
    \centering
